...
function paddle() {
  ...
  this.s = 11;       // scrolling paddle speed
}

//Manage Game Mechanics
function gameStart() {
  io.sockets.emit('dialog', {
    "action": "hide"
  });
...
function gameMove() {
  var isCollide = ballMove(),    //Move ball function
    lp = players[players.left],
    rp = players[players.right],
    obj = {};

  if (lp && rp) {
    //If Cheatmode=true and , bloat Ball!
    ...
    obj = {
      ...
      "paddle": lp.paddle,
      "oppPaddle": rp.paddle,
      "score": [lp.score, rp.score],
      "collide": isCollide
    };
    io.sockets.in('players')
      .emit('game', obj);
    io.sockets.in('observers')
      .volatile.emit('game', obj);
  }
}
...
//    h: 30,
  ...
  // combined they give either 1 or -1
  // 1 & -1 ~ true or false
  // * 12 ?
  };
}
...
function ballMove() {
  var B = ball,                      //   ball = defaultBall()
    lp = players[players.left],      // left and right player assigned Players Object
    rp = players[players.right],     // players = { length: 0,left: false,right: false},
    c = false;                       // colision?

  B.x += B.s;
  B.y += B.sy;                       // set ball x & y cordinates by iterating randomly ?
...
// only collides when B hits P
	// B.x ~ P.x      B.y ~ P.y
	  // if Ball position is < Paddles position + width &&
	  // Ball position x + Ball width is > Paddle postion x ......
...
io.sockets.on('connection', function (socket) {
  ...
  if (players.length < 2) {
    ...
    if (!players.left) {
      ...
      socket.emit('dialog', {
        "msg": "You're playing on the left side!",
        "action": "show"
      });
    } else {
      ...
      socket.emit('dialog', {
        "msg": "You're playing on the right side!",
        "action": "show"
      });
    }
    ...
    if (players.length == 2) {
      io.sockets.emit('dialog', {
        "msg": "Both players ready, starting game in about 5 seconds!",
        "action": "show"
      });
      setTimeout(gameStart, 5000);
    } else socket.emit('dialog', {
      "msg": "We're still waiting on another player!",
      "action": "show"
    });
  } else {
    socket.emit('dialog', {
      "msg": "Sorry, the room is full, feel free to stick around and watch the match!",
      "action": "show"
    });
  ...
  socket.on('receive', function (data) {
    ...
    if (data === 'get_dimensions') socket.emit('game', {
      "canvas": canvas
    });
    //Handle Chat Stuff
